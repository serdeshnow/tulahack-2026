...
class PostgresDatabase:
    ...
    def _connect(self):
        try:
            ...
            raise RuntimeError("psycopg is required for the postgres database backend") from exc
    ...
    def _init_schema(self) -> None:
        statements = (
            ...
            CREATE TABLE IF NOT EXISTS job_results (
                ...
                has_redacted_audio BOOLEAN NOT NULL DEFAULT FALSE
            )
            """,
    ...
    def update_upload_session(self, session: UploadSession) -> None:
        with self._connect() as connection, connection.cursor() as cursor:
            cursor.execute(
                ...
                SET filename = %s, content_type = %s, size_bytes = %s, object_key = %s, upload_token = %s,
                    created_at = %s, expires_at = %s, status = %s, checksum = %s, bytes_received = %s,
                    metadata_json = %s
                WHERE upload_id = %s
                """,
                (
    ...
    def get_upload_session(self, upload_id: str) -> UploadSession | None:
        with self._connect() as connection, connection.cursor() as cursor:
            cursor.execute("SELECT * FROM upload_sessions WHERE upload_id = %s", (upload_id,))
            row = cursor.fetchone()
    ...
    def update_job(self, job: JobRecord) -> None:
        with self._connect() as connection, connection.cursor() as cursor:
            cursor.execute(
                """
                UPDATE jobs
                SET upload_id = %s, profile_json = %s, webhook_url = %s, idempotency_key = %s, status = %s,
                    stage = %s, progress = %s, trace_id = %s, created_at = %s, updated_at = %s,
                    last_error = %s, retry_count = %s, quality_flags_json = %s
                WHERE job_id = %s
                """,
                (
                    job.upload_id,
    ...
    def count_jobs_by_status(self) -> dict[str, int]:
        with self._connect() as connection, connection.cursor() as cursor:
            cursor.execute("SELECT status, COUNT(*) AS total FROM jobs GROUP BY status")
            rows = cursor.fetchall()
    ...
    def list_stage_executions(self, job_id: str) -> list[StageExecution]:
        with self._connect() as connection, connection.cursor() as cursor:
            cursor.execute("SELECT * FROM stage_executions WHERE job_id = %s", (job_id,))
            rows = cursor.fetchall()
    ...
    def list_artifacts(self, job_id: str) -> list[ArtifactRecord]:
        with self._connect() as connection, connection.cursor() as cursor:
            cursor.execute("SELECT * FROM artifacts WHERE job_id = %s ORDER BY created_at", (job_id,))
            rows = cursor.fetchall()
        return [self._row_to_artifact(row) for row in rows]

    def count_artifacts_by_access_level(self) -> dict[str, int]:
        with self._connect() as connection, connection.cursor() as cursor:
            cursor.execute("SELECT access_level, COUNT(*) AS total FROM artifacts GROUP BY access_level")
            rows = cursor.fetchall()
    ...
    def get_artifact(self, job_id: str, kind: ArtifactKind, variant: str) -> ArtifactRecord | None:
        with self._connect() as connection, connection.cursor() as cursor:
            cursor.execute(
                "SELECT * FROM artifacts WHERE job_id = %s AND kind = %s AND variant = %s",
    ...
    def list_expired_artifacts(self, now: datetime) -> list[ArtifactRecord]:
        with self._connect() as connection, connection.cursor() as cursor:
            cursor.execute(
                "SELECT * FROM artifacts WHERE expires_at IS NOT NULL AND expires_at <= %s",
                (now.isoformat(),),
            )
            rows = cursor.fetchall()
        return [self._row_to_artifact(row) for row in rows]
    ...
    def delete_artifact(self, artifact_id: str) -> None:
        with self._connect() as connection, connection.cursor() as cursor:
            cursor.execute("DELETE FROM artifacts WHERE artifact_id = %s", (artifact_id,))
            connection.commit()

    def replace_events(self, job_id: str, events: Sequence[dict[str, object]]) -> None:
        with self._connect() as connection, connection.cursor() as cursor:
            cursor.execute("DELETE FROM events WHERE job_id = %s", (job_id,))
            for event in events:
                cursor.execute(
                    """
                    INSERT INTO events (
    ...
    def list_events(self, job_id: str) -> list[dict[str, object]]:
        with self._connect() as connection, connection.cursor() as cursor:
            cursor.execute("SELECT * FROM events WHERE job_id = %s ORDER BY start_ms, end_ms", (job_id,))
            rows = cursor.fetchall()
        return [
            {
    ...
    def list_model_runs(self, job_id: str) -> list[ModelRun]:
        with self._connect() as connection, connection.cursor() as cursor:
            cursor.execute("SELECT * FROM model_runs WHERE job_id = %s ORDER BY created_at", (job_id,))
            rows = cursor.fetchall()
    ...
    def get_job_result(self, job_id: str) -> JobResultRecord | None:
        with self._connect() as connection, connection.cursor() as cursor:
            cursor.execute("SELECT * FROM job_results WHERE job_id = %s", (job_id,))
            row = cursor.fetchone()
    ...
    def list_job_results(
        ...
        *,
        ...
        if processing_profile:
            clauses.append("processing_profile = %s")
        ...
        query = f"SELECT * FROM job_results {where} ORDER BY created_at DESC LIMIT %s OFFSET %s"
    ...
    def _row_to_upload(self, row: dict) -> UploadSession:
    ...
    def _row_to_stage(self, row: dict) -> StageExecution:
    ...
    def _row_to_artifact(self, row: dict) -> ArtifactRecord:
    ...
    def _row_to_model_run(self, row: dict) -> ModelRun:
        return ModelRun(
            run_id=row["run_id"],
            job_id=row["job_id"],
    ...
    def _row_to_job_result(self, row: dict) -> JobResultRecord:
