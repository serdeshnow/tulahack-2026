...
export const ENTITY_TAG_COLOR_VARS: Record<EntityType, `--tag-${string}`> = {
  ...
  RU_PASSPORT_ISSUER: '--tag-passport',
  RU_INN: '--tag-inn',
  RU_SNILS: '--tag-snils',
  PHONE: '--tag-phone',
  EMAIL: '--tag-email',
  ADDRESS: '--tag-address',
  CARD_NUMBER: '--tag-card-information',
...
export const ENTITY_TAG_CLASSNAMES: Record<EntityType, string> = {
  ...
  RU_PASSPORT_ISSUER: 'border-transparent bg-[var(--tag-passport)] text-slate-800',
  ...
  CARD_NUMBER: 'border-transparent bg-[var(--tag-card-information)] text-slate-800',
